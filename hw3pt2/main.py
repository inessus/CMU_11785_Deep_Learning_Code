...
"""
Phoneme distribution calculated by
counters = [0] * 46
...
"""
PHONEME_DISTRIBUTION = np.array([0.0,  #blank
    0.00823438323519969, 0.0015284028190540885, 0.0023780179298431956, 0.007730944831290355, 0.00012359865006157322,
    0.00518360679953354, 0.018875925545379042, 0.018511158797636353, 0.10517340739873578, 0.011238935785476958,
    0.003987312548327826, 0.013510437805510993, 0.016689837226607072, 0.0041837640612305705, 0.04370820066994488,
    0.019121615544891682, 0.0274268419087852, 0.028692974421611072, 0.016100985121435672, 0.017049579638981405,
    0.006578362297789343, 0.010565172412580332, 0.061547103395295115, 0.03578180919282545, 0.005560431952160288,
    0.038147266284247756, 0.03664499000910912, 0.029672217384903783, 0.06864196736895152, 0.007871123788067504,
    0.012637710752027445, 0.0017972047612611684, 0.0264420721765873, 0.04339819917771728, 0.04788292092751241,
    0.008666978510415197, 0.03537935992982008, 0.06419392326734538, 0.004766084530423104, 0.002682492653165608,
    0.012076994924918845, 0.017149061479274866, 0.014272629480890694, 0.0082655341144835, 0.029223041802972697,
    0.0007054166857172716])
...
def get_data_loaders(args, use_dev_for_train=False):
    ...
    ydev = np.load(args.data_dir + '/dev_phonemes.npy')
    if use_dev_for_train:
        xtrain = np.load(args.data_dir + '/dev.npy')
        ytrain = np.load(args.data_dir + '/dev_phonemes.npy')
    else:
        xtrain = np.load(args.data_dir + '/train.npy')
        ytrain = np.load(args.data_dir + '/train_phonemes.npy')
...
class MyModel(nn.Module):
    def __init__(self, nhid, nlayers, dropout=0, dropouth=0, dropouti=0, n_input=40, n_output=47):
